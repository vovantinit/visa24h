...
class to_cash_book_s03_dn(models.Model):
    ...
    def check_report_xls(self, data):
        ...
        amount_currency_sum = self.account.get_report_opening_balance_nt(data)
        ...
        for line in res:
            ...
            if i == len(res)-1 or line['ldate'] != res[i+1]['ldate']:
                ...
                line['amount_currency_sum'] = self.account.get_current_balance_nt(data['form']['date_from'], line['ldate'], data, self.target_move)
...
class to_cash_book_s07_dn(models.Model):
    ...
    @api.model
    def check_report_xls(self, data):
        res = self.cash_account.get_report_move_lines(data)
        ...
        rows = 4
        amount_currency = 0
        init_balance = self.cash_account.get_report_opening_balance(data)
        amount_currency_sum = self.cash_account.get_report_opening_balance_nt(data)
        ...
        for line in res:
            ...
            line['amount_currency_sum'] = ''
            if i == len(res)-1 or line['ldate'] != res[i+1]['ldate']:
                line['progress_v2'] = self.cash_account.get_current_balance(data['form']['date_from'], line['ldate'], data, self.target_move)
                line['amount_currency_sum'] = self.cash_account.get_current_balance_nt(data['form']['date_from'], line['ldate'], data, self.target_move)
